...
def scrape_from_major(major: Major):
    ...
    # If hash is the same as the last snapshot, that means that the page has not changed and we skip
    ...
    for school_class in school_classes:
        ...
        if created:
            logger.debug(f"New professor found: {professor_full_name}. Added to the database.")
        ...
        if class_:
            ...
            logger.debug(f"New class found: {class_name}. Added to the database.")
        ...
        if table_length < 2:
            logger.debug(f"Table for {class_name} is empty. Skipping.")
        ...
        for i in range(1, len(rows)):
            ...
            if exam_code:
                ...
                logger.warning(
                    f"Could not extract exam code for {exam_name} in {class_name}. Skipping."
                )
                continue
            ...
            if not exam_name:
                ...
                logger.warning(
                    f"No exam name found for exam code {exam_code} in {class_name}."
                    " Using default name."
                )
